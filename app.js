...
async function loadAIProviders(){
  ...
  try{
    ...
    const names={groq:'Groq GPT-OSS 120B',gemini:'Google Gemini Flash',openrouter:'OpenRouter Free',groq_qwen:'Groq Qwen 3.6 27B',mistral:'Mistral',cerebras:'Cerebras',cloudflare:'Cloudflare Workers AI',deepseek:'DeepSeek V4.1 Flash',openai:'OpenAI',huggingface:'Hugging Face'};
...
async function loadHistory(){
  ...
  try{
    const hq=new URLSearchParams(historyQuery().replace(/^\?/,'&')); hq.set('symbol',symbol); hq.set('limit','200'); const d=await api(`/api/v1/signals/history?${hq.toString()}`);
...
async function autoEntryTick(){if(!token)return;try{const target=symbol;const d=await api(`/api/v1/signals/auto-record?symbol=${encodeURIComponent(target)}&interval=${encodeURIComponent(interval)}`,{method:'POST'});const total=Number(d.queued||0);if(total){showToast(`AUTO TRADE: ${total} ta signal MT5 navbatiga uzatildi`);loadStats();loadHistory();}else{loadStats();}}catch(e){console.warn('AUTO ENTRY',e)}}
...
async function loadMT5Status(){ if(!IS_ADMIN)return;
  ...
  setText('mt5DualState',dual?'🟢 BOTH SYMBOLS':'⚪ SINGLE SYMBOL');
  const dualOn=$('mt5DualOn'),dualOff=$('mt5DualOff'); if(dualOn) dualOn.classList.toggle('primary',dual); if(dualOff) dualOff.classList.toggle('primary',!dual);
  setText('mt5DualInfo',dual?'XAUUSD + EURUSD bir vaqtda ishlaydi. Bitta dual-symbol EA ikkalasini ham bajaradi.':'Hozir faqat tanlangan sahifadagi symbol auto trade qiladi.');
}
