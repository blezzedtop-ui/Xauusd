...
async function loadCalendar(){try{const d=await api('/api/v1/calendar?days=7');const source=d.provider?` · Source: ${d.provider}`:'';if(!d.events?.length){$('calendar').innerHTML=`<div class="mini">${d.warning||'HIGH IMPACT eventlar topilmadi.'}</div>`;return}$('calendar').innerHTML=`<div class="mini" style="margin-bottom:8px">USD · ALL IMPACT${source} · ${d.events.length} event</div>`+d.events.map(x=>`<div class="calendar-item"><div><span class="tag">${x.impact||'MEDIUM'}</span> · ${x.country||'USD'} · ${x.time||''}</div><b>${x.event||'Economic event'}</b><div class="mini">Forecast: ${x.forecast??x.estimate??'—'} · Previous: ${x.previous??'—'} · Actual: ${x.actual??'—'} ${x.unit||''}</div></div>`).join('')}catch(e){$('calendar').innerHTML=`<div class="mini">Calendar ma’lumoti hozircha mavjud emas: ${e.message}</div>`}}
...
async function loadAutoSignals(){
  try{
    ...
    $('autoSignalsGrid').innerHTML=order.map(tf=>{
      ...
      return `<div class="signal-box auto-signal"><div class="section-head"><b>${tfName(tf)}</b><span class="pill">${x.confidence||0}%</span></div><div class="signal-main ${cls}">${sig}</div><div class="mini">Entry: ${fmt(x.entry)} · SL: ${fmt(x.stop_loss)} · TP: ${(x.take_profit||[]).map(fmt).join(' / ')||'—'}</div><div class="mini">${x.reason||'Live engine'}</div></div>`;
    }).join('');
