...
async function autoEntryTick(){if(!token)return;try{const dual=localStorage.getItem('signalx_mt5_dual_mode')==='1';const symbols=dual?['XAU/USD','EUR/USD']:[symbol];let total=0;for(const s of symbols){const d=await api(`/api/v1/signals/auto-record?symbol=${encodeURIComponent(s)}`,{method:'POST'});total+=Number(d.count||0);}if(total){showToast(`AUTO ENTRY: ${total} ta savdo navbatga qo‘yildi`);loadStats();loadHistory();}else{loadStats();}}catch(e){console.warn('AUTO ENTRY',e)}}
